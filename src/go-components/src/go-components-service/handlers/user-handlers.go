...
// Index Handler
func NNewIndex(w http.ResponseWriter, r *http.Request) {
...
func UserIndex(w http.ResponseWriter, r *http.Request) {
	...
	if end > len(repos.Users) {
		end = len(repos.Users)
	}

	ret := make([]models.User, 0, count)

	idx := offset
	for len(ret) < count && idx < len(repos.Users) {
		// Do NOT return any users with an associated identity ID.
		if len(repos.Users[idx].IdentityId) == 0 {
			ret = append(ret, repos.Users[idx])
		}
		idx++
	}
...
func UserShowByID(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(repos.RepoFindUserByID(userID)); err != nil {
		panic(err)
	}
}
...
func UserShowByUsername(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(repos.RepoFindUserByUsername(username)); err != nil {
		panic(err)
	}
}
...
func UserShowByIdentityId(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(repos.RepoFindUserByIdentityID(identityID)); err != nil {
		panic(err)
	}
}
...
func ClaimUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(repos.RepoClaimUser(userId)); err != nil {
		panic(err)
	}
}
...
func GetRandomUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(repos.RepoFindRandomUser(count)); err != nil {
		panic(err)
	}
}
...
func GetUnclaimedUsers(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(repos.RepoFindRandomUsersByPrimaryPersonaAndAgeRange(primaryPersona, ageRange, count)); err != nil {
		panic(err)
	}
}
...
func UserCreate(w http.ResponseWriter, r *http.Request) {
	...
	t, err := repos.RepoCreateUser(user)
	if err != nil {
		panic(err)
	}
...
func CreateEndpointAndSendConfirmation(w http.ResponseWriter, r *http.Request, updateEndpointInput *pinpoint.UpdateEndpointInput, phonenumber string) {
	enableCors(&w)
	updateEndpointOutput, err := util.Pinpoint_client.UpdateEndpoint(updateEndpointInput)
	if err != nil {
		...
		getLongCodeResponse, err := util.Ssm_client.GetParameter(&ssm.GetParameterInput{
		...
		if err != nil {
			...
			sendMessageInput = &pinpoint.SendMessagesInput{
				ApplicationId: &util.Pinpoint_app_id,
			...
			sendMessageOutput, err := util.Pinpoint_client.SendMessages(sendMessageInput)
			if err != nil {
...
func UserVerifyAndUpdatePhone(w http.ResponseWriter, r *http.Request) {
	...
	user := repos.RepoFindUserByID(userDetails.UserID)
	if user.ID == "" {
		...
		res, err := util.Pinpoint_client.PhoneNumberValidate(phoneValidateInput)
		if err != nil {
