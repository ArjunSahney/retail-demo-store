...
from common.common import BadRequest
import logging
...
def fetch_product_details(item_ids: Union[str, List[str]], fully_qualify_image_urls=False) -> List[Dict]:
    ...
    logger.debug(f"Asking for product info from {url}")
...
@recommendations_blueprint.errorhandler(BadRequest)
...
@recommendations_blueprint.route('/health')
def health():
    return 'OK'

@recommendations_blueprint.route('/related', methods=['GET'])
def related():
    ...
    logger.info("Filter SSM for /related: %s", filter_ssm)
    ...
    try:
        ...
        if rerank_items:
            logger.info('Reranking related items to personalize order for user %s', user_id)
        ...
        logger.exception('Unexpected error generating related items', e)
...
def recommendations():
    ...
    logger.info(f"Filter SSM for /recommendations: {filter_ssm}")
    ...
    try:
        ...
        logger.debug("Recommendations response to be returned: %s", response)
        return response

    except Exception as e:
        logger.exception('Unexpected error generating recommendations', e)
        raise BadRequest(message = 'Unhandled error', status_code = 500)

@recommendations_blueprint.route('/popular', methods=['GET'])
def popular():
    """ Returns item/product recommendations for a given user in the context
    of a current item (e.g. the user is viewing a product and I want to provide
    recommendations for other products they may be interested in).
...
def ranking_request_params():
    ...
    logger.info(f"JSON payload: {content}")
    ...
    logger.info(f"Items pulled from json: {items}")
...
def get_ranking(user_id, items, feature,
    ...
    logger.info(f"Items given for ranking: {items}")
    ...
    logger.info(f"Unranked items: {unranked_items}")
    ...
    if experiment:
        logger.info('Using experiment: %s', experiment.name)
        ...
        logger.debug("Experiment ranking resolver gave us this ranking: %s", ranked_items)
        ...
        logger.info(f'Falling back to Personalize: {values}')
        ...
        if inference_arn:
            ...
            logger.info(f'Falling back to No-op: {values}')
...
@recommendations_blueprint.route('/rerank', methods=['POST'])
def rerank():
    """
    ...
    try:
        ...
        logger.debug(f"Response items for reranking: {response_items}")
        resp = Response(json.dumps(response_items, cls=CompatEncoder), content_type='application/json',
                        headers=resp_headers)
        return resp
    except Exception as e:
        logger.exception('Unexpected error reranking items', e)
...
def get_top_n(user_id, items, feature, top_n,
    ...
    logger.info(f"Items given for top-n: {items}")
    ...
    logger.info(f"Pre-selection items: {unranked_items}")
    ...
    if experiment:
        ...
        logger.debug(f"Experiment ranking resolver gave us this ranking: {topn_items}")
...
@recommendations_blueprint.route('/choose_discounted', methods=['POST'])
def choose_discounted():
    ...
    try:
        ...
        logger.exception('Unexpected error calculating discounted items', e)
        return json.dumps(items)
...
@recommendations_blueprint.route('/coupon_offer', methods=['GET'])
def coupon_offer():
    """
    ...
    try:
        ...
        if not offers_response.ok:
            logger.exception('Offers service did not return happily', offers_response.reason())
            raise BadRequest(message='Cannot obtain offers', status_code=500)
        else:
            ...
            if not inference_arn:
                logger.warning('No campaign Arn set for offers - returning arbitrary')
        ...
        logger.debug(f"Recommendations response to be returned for offers: {resp}")
        return resp
...
@recommendations_blueprint.route('/experiment/outcome', methods=['POST'])
def experiment_outcome():
    ...
    try:
        ...
        logger.exception('Unexpected error logging outcome', e)
        raise BadRequest(message='Unhandled error', status_code=500)
