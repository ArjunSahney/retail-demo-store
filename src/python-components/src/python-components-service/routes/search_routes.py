...
@search_blueprint.errorhandler(BadRequest)
def handle_bad_request(error):
    response = jsonify(error.to_dict())
    response.status_code = error.status_code
    return response

@search_blueprint.route('/checkSearch')
def index():
    return 'Search Service'

@search_blueprint.route('/search/products', methods=['GET'])
def search_products():
    ...
    logger.info('Searching products for "%s" starting at %d and returning %d hits with collapse size of %d',
    ...
    try:
        ...
        logger.debug('Total hits across categories: %d', total_hits)

        cats_with_hits = len(results["hits"]["hits"])
        avg_hits_cat = int(size / cats_with_hits) if cats_with_hits > 0 else 0
        logger.debug('Average hits per category: %d', avg_hits_cat)
        hits_for_cats = []
        accum_hits = 0
        cats_with_more = 0
        ...
        if e.error == INDEX_DOES_NOT_EXIST:
            logger.error('Search index does not exist')
        ...
        logger.exception('Unexpected error performing product search', e)
        raise BadRequest(message = 'Unhandled error', status_code = 500)

@search_blueprint.route('/similar/products', methods=['GET'])
def similar_products():
    product_id = request.args.get('productId')
    if not product_id:
        raise BadRequest('productId is required')
    offset, size = get_offset_and_size(request)
    logger.info(f'Searching for similar products to "{product_id}" starting at {offset} and returning {size} hits')
    ...
    try:
        ...
        logger.exception('Unexpected error performing similar product search', e)
        raise BadRequest(message = 'Unhandled error', status_code = 500)
