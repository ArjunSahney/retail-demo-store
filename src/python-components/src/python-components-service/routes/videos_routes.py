...
def load_default_streams_config(app):
...
# -- Video streaming
def download_video_file(s3_key, app):
    """
...
def start_streams(app):
    ...
    if is_ssm_parameter_set(SSM_VIDEO_CHANNEL_MAP_PARAM) and not USE_DEFAULT_IVS_STREAMS:
        ...
        stream_details = load_default_streams_config(app)


def stream(s3_video_key, ivs_channel_arn, channel_id, app):
    """
        ...
        endpoint.
    """
    video_filepath, thumb_url = download_video_file(s3_video_key, app)
            ...
            while True:
                try:
                    ...
                    while True:
                        ...
                        send_text += text
                    put_ivs_metadata(ivs_channel_arn, send_text, app)
...
@videos_blueprint.errorhandler(BadRequest)
...
@videos_blueprint.route('/checkVideos')
def index():
    return 'Videos Service'


@videos_blueprint.route('/stream_details')
def streams():
    response_data = []
